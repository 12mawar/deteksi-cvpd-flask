...
def statistik():
    ...
    cursor.execute("SELECT COUNT(*) AS total FROM hasil_deteksi_cvpd WHERE username=%s", (session['user'],))
    total_deteksi = cursor.fetchone()['total']

    cursor.execute("SELECT COUNT(*) AS total FROM hasil_deteksi_cvpd WHERE prediksi='Sehat' AND username=%s",
                   (session['user'],))
    total_sehat = cursor.fetchone()['total']

    cursor.execute("SELECT COUNT(*) AS total FROM hasil_deteksi_cvpd WHERE prediksi='Terinfeksi CVPD' AND username=%s",
                   (session['user'],))
    total_terinfeksi = cursor.fetchone()['total']
